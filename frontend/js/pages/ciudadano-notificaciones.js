...
(function () {
  ...
  function render() {
    ...
    if (notificaciones.length === 0) {
      contenedor.innerHTML = `<div class="card"><div class="empty-state"><div class="empty-state-icon">🔔</div><h3>No tienes notificaciones</h3><p>Aquí verás avisos sobre recolecciones e incidencias.</p></div></div>`;
      return;
    }
      ...
      <div class="card" style="border-left:4px solid ${n.leida ? 'var(--color-border)' : 'var(--color-primary-500)'}; cursor:pointer;" data-id="${n.id}">
        <div class="flex justify-between items-center" style="align-items:flex-start;">
          <div class="flex gap-3" style="align-items:flex-start;">
            <span style="font-size:1.3rem;">${ICONOS[n.tipo] || 'ℹ️'}</span>
            <div>
